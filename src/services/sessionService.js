...
export default class SessionService {
  ...
  login = ({ body, callbackSuccess, callbackError }) => {
    const data = {
      url: `${process.env.REACT_APP_BASE_URL}${process.env.REACT_APP_LOGIN}`,
  ...
  current = ({ callbackSuccess, callbackError }) => {
    const data = {
      url: `${process.env.REACT_APP_BASE_URL}${process.env.REACT_APP_CURRENT}`,
      config: getAuthHeaders(),
      callbackSuccess,
      callbackError,
    };
  ...
  register = ({ body, callbackSuccess, callbackError }) => {
    const data = {
      url: `${process.env.REACT_APP_BASE_URL}${process.env.REACT_APP_REGISTER}`,
      config: getAuthHeaders(),
      body,
      callbackSuccess,
      callbackError,
